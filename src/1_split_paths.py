...
cat = ["Constants", "Authentication", "Self", "Monitor", "Configuration", "Installer"]
verbs = ["get", "post", "put", "delete"]
order = ["openapi", "info", "servers", "security", "tags", "paths", "components"]
#order = ["openapi", "info", "servers", "security", "tags", "components"]
...
cat_responses = {
    ...
    "authentication": [],
    "self": [],
    "monitor": {"msps": [], "orgs": [], "sites": [], "admins": []},
    "configuration": {"msps": [], "orgs": [], "sites": [], "admins": []},
    "installer": [],
    "webhook": [],
}
...
def add_endpoint(
    ...
    if "Authentication" in endpoint.get("tags"):
        endpoint["tags"].remove("Authentication")
    if "Monitor" in endpoint.get("tags"):
        endpoint["tags"].remove("Monitor")
    if "Configuration" in endpoint.get("tags"):
        endpoint["tags"].remove("Configuration")
    if "Self" in endpoint.get("tags"):
        endpoint["tags"].remove("Self")

    cat_path[path][verb] = endpoint


def select_scope(endpoint: dict):
    if endpoint.get("operationId") in [
        "verifyAdminInvite",
        "activateSdkInvite",
        "registerNewAdmin",
        "verifyRegistration",
        ...
        return "orgs"
    elif endpoint.get("operationId") in [
...
def split_scope(category: str, path: str, parameters: dict, verb: str, endpoint: dict):
    ...
    if scope in ["msps", "orgs", "sites"]:
        ...
        add_endpoint(category, path, parameters, verb, endpoint, "self")
    else:
        ...
        for verb in properties:
            if verb in verbs:
                ...
                if path.startswith("/api/v1/installer/"):
                    ...
                    add_endpoint(
                        "authentication", path, parameters, verb, properties[verb]
                    )
                elif "Self" in properties[verb]["tags"]:
                    add_endpoint("self", path, parameters, verb, properties[verb])
                    ...
                    split_scope(
                        "configuration", path, parameters, verb, properties[verb]
                    )
                else:
...
def custom_paths_oder(tags: list, paths: dict):
    ...
    for tag in tags:
        ...
        tmp_paths = {
            ...
            if (val.get("get") and tag in val["get"].get("tags"))
            or (val.get("post") and tag in val["post"].get("tags"))
            or (val.get("put") and tag in val["put"].get("tags"))
            or (val.get("delete") and tag in val["delete"].get("tags"))
        }
...
def save_file(
    ...
    data = {
        ...
        "tags": tags,
        "paths" : custom_paths_oder(cat_tag, cat_path),
        "components": {
    ...
    with open(filename, "w") as oas_out_file:
        paths_order = custom_paths_oder(cat_tag, cat_path)
        for item in order:
            yaml.dump({item: data[item]}, oas_out_file, sort_keys=False)
        # oas_out_file.write("paths:\r\n")
        # for p in paths_order:
        #     yaml.dump({p: cat_path[p]}, oas_out_file=,)
